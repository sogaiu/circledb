...
(defn- next-id
  "Returns a pair composed of the id to use for the given entity and the
  next free running id in the database"
  [db ent]
...
(defn- update-attr-value
  "Updating the attribute value based on the kind of the operation, the
  cardinality defined for this attribute and the given value"
  ...
  (cond
    ...
    , (assoc attr :value value)
    (= :db/add operation)
    , (assoc attr :value (cset/union (:value attr)  value))
    (= :db/remove operation)
    , (assoc attr :value (cset/difference (:value attr) value))))

(defn- update-creation-ts
  "Updates the timestamp value of all the attributes of an entity to the
  given timestamp"
  [ent ts-val]
  (reduce #(assoc-in %1
                     [:attrs %2 :ts] ts-val)
...
(defn- update-entry-in-index
  [index path _operation]
  (let [update-path (butlast path)
        update-value (last path)
        to-be-updated-set (get-in index update-path #{})]
    (assoc-in index
              update-path (conj to-be-updated-set update-value))))

(defn- update-attr-in-index
  [index ent-id attr-name target-val operation]
  (let [colled-target-val (cnstr/collify target-val)
        update-entry-fn
        (fn [indx vl]
          (update-entry-in-index indx
                                 ((cnstr/from-eav index) ent-id attr-name vl)
                                 operation))]
    (reduce update-entry-fn
            index
            colled-target-val)))
...
(defn- add-entity-to-index
  ...
  (let [ent-id (:id ent)
        ...
        relevant-attrs (filter #((cnstr/usage-pred index) %)
                               all-attrs)
        add-in-index-fn (fn [ind attr]
                          (update-attr-in-index ind
                                                ...
                                                :db/add))]
    (assoc layer
           ind-name (reduce add-in-index-fn
                            index
                            relevant-attrs))))
...
(defn add-entity
  ...
  (let [[fixed-ent next-top-id] (fix-new-entity db ent)
        ...
        new-layer (reduce add-fn layer-with-updated-storage (cnstr/indices))]
    (assoc db
...
(defn- remove-entry-from-index [index path]
  (let [path-head (first path)
    ...
    (cond
      ;; the set of items does not contain the item to remove, => nothing
      ;; to do here
      (not (contains? old-entries-set val-to-remove))
      , index
      ;; a path that splits at the second item - just remove the unneeded
      ;; part of it
      (= 1 (count old-entries-set))
      , (update-in index
                   [path-head] dissoc (second path))
      :else
      , (update-in index
                   path-to-items disj val-to-remove))))
...
(defn- remove-entries-from-index
  ...
  (if (= operation :db/add)
    ...
    (let [attr-name (:name attr)
          datom-vals (cnstr/collify (:value attr))
          paths (map #((cnstr/from-eav index) ent-id attr-name %)
                     datom-vals)]
      (reduce remove-entry-from-index
              index
              paths))))

(defn- update-index
  [ent-id old-attr target-val operation layer ind-name]
  (if-not ((cnstr/usage-pred (get-in layer [ind-name])) old-attr)
    layer
...
(defn- put-entity
  ...
  (assoc-in (stor/get-entity storage e-id)
            [:attrs (:name new-attr)] new-attr))

(defn- update-layer
  [layer ent-id old-attr updated-attr new-val operation]
  (let [storage (:storage layer)
        new-layer (reduce (partial update-index
                                   ent-id old-attr new-val operation)
                          layer
                          (cnstr/indices))]
    (assoc new-layer
           :storage (stor/write-entity storage
                                       (put-entity storage
                                                   ent-id updated-attr)))))
...
(defn update-entity
  ...
  ([db ent-id attr-name new-val operation]
     (let [update-ts (next-ts db)
           ...
           attr (cnstr/attr-at db ent-id attr-name)
           updated-attr (update-attr attr
                                     new-val
                                     update-ts
                                     operation)
           fully-updated-layer (update-layer layer
                                             ent-id
                                             attr
                                             updated-attr
                                             new-val
...
(defn- remove-entity-from-index [ent layer ind-name]
  (let [ent-id (:id ent)
        ...
        remove-from-index-fn (partial remove-entries-from-index
                                      ...
                                      :db/remove)]
    (assoc layer
           ind-name (reduce remove-from-index-fn index relevant-attrs))))
...
(defn transact-on-db [initial-db ops]
  (loop [[op & rst-ops] ops
    ...
    (if op
      (recur rst-ops
             (apply (first op) transacted (rest op)))
  ...
  (when ops
    (loop [[frst-op# & rst-ops#] ops
      ...
      (if frst-op#
        (recur rst-ops#
               res#
               (conj accum-ops# (vec frst-op#)))
        (list* (conj res# accum-ops#))))))

(defn- _what-if
  "Operates on the db with the given transactions, but without
  eventually updating it"
  [db f ops]
  (f db ops))
...
(defmacro transact [db-conn & ops]
  `(_transact ~db-conn swap! ~@ops))

(defn evolution-of
  "The sequence of the values of an entity's attribute, as changed
  through time"
  [db ent-id attr-name]
  (loop [res []
         ts (:curr-time db)]
    (if (= -1 ts)
      (reverse res)
      (let [attr (cnstr/attr-at db ent-id attr-name ts)]
